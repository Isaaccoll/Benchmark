...
namespace BenchmarkRpi
{
    ...
    class busSpeed
    {
        int[] array = new int[16779000]; // 64 MB + 1000
        char[,] configdata = new char[2,1024];
        int andsum1;
        int andsumx = 0x55555555;
        uint wordBytes = 4;

        int memoryWords;
        uint[] memoryKBytes = new uint[20];
        double memoryMB;

        int[] mbpersec = new int[20];
       ...
       public void checkTime()
        ...
        public double calcPass()
        {
            int i, g, p;
            int j, tt0;
            int endTest;

            double[] addressInc = new double[10];
            ...
            if (andsum1 != andsumx)
            {
               //(resultchars, " Wrong result Expected 0x%8.8x Was 0x%8.8x at %d KB\n", andsumx, andsum1, memoryKBytes[ipass]);
                andsum1 = andsumx;
            }
            else
            {
